...
{
    size_t gms = sizeof(MPOS_Grid) + rect.w * rect.h * sizeof(MPOS_GridTile);
...
void MPOS_CalcGrid(MPOS_Grid *grid)
{
    ...
    for (uint32_t row = 0; row < grid->rect.h; row++)
    {
        ...
        {
            grid->tiles[key].key = key;
            grid->tiles[key].rect.x = col * grid->t_w;
            grid->tiles[key].rect.y = row * grid->t_h;
            grid->tiles[key].rect.w = grid->t_w;
            grid->tiles[key].rect.h = grid->t_h;
            grid->tiles[key].row = row;
            grid->tiles[key].col = col;
...
MPOS_Grid * MPOS_CreateIsoGrid(SDL_Rect rect, u_int32_t t_w , u_int32_t t_h)
{
    ...
    uint32_t keys = 0;
    for (size_t row_nb = 0; row_nb < t_h; row_nb++)
    {
        keys += rect.w / 2;
        if (row_nb < t_h - 1)
            keys += rect.w / 2 - 1;
...
void MPOS_CalcIsoGrid(MPOS_Grid *grid)
{
    ...
    for (size_t row_nb = 0; row_nb < grid->rect.h; row_nb++)
    {
        for (size_t col_nb = 0; col_nb < grid->rect.w / 2; col_nb++)
        {
            ...
            y = grid->t_h * row_nb;
            grid->tiles[key].key = key;
            grid->tiles[key].rect.x = x;
            grid->tiles[key].rect.y = y;
            grid->tiles[key].rect.w = grid->t_w;
            grid->tiles[key].rect.h = grid->t_h;
            grid->tiles[key].row = row;
            grid->tiles[key].col = col;
